...
class LeafNode extends BPlusNode {
    ...
    // copy another empty leafNode
    // move the data to the new Node
    private long OrganizePagination(DataBox key, RecordId rid) {
        int splitKeysIndex = keys.size() / 2 ;
        int splitRidsIndex = rids.size() / 2 ;
        List<DataBox> newKeys = keys.subList(splitKeysIndex, keys.size());
        List<RecordId> newRids = rids.subList(splitRidsIndex, rids.size());
        newKeys.add(key);
        newRids.add(rid);
        // get the leftNode the right split leaf id ant put to the new Node the right split id(the cursor transfer)
        LeafNode newNode = new LeafNode(metadata, bufferManager, newKeys, newRids, this.rightSibling, treeContext);
        keys = keys.subList(0, splitKeysIndex);
        rids = rids.subList(0, splitRidsIndex);
        long newRightSibling = newNode.getPage().getPageNum();
        // set the left node id is the new Node id
        this.rightSibling = Optional.of(newRightSibling);
     ...
     * must to read the toBytes just like the node serialize
     */
